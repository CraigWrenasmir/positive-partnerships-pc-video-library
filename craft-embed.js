(function () {
  ...
  const dataUrl = "https://craigwrenasmir.github.io/positive-partnerships-pc-video-library/data.json";
  const downloads = {
...
#pp-pc-library-embed h2,#pp-pc-library-embed h3,#pp-pc-library-embed h4{margin:0;line-height:1.06;font-family:Georgia,serif;color:var(--brand-plum)}
...
@media (max-width:960px){#pp-pc-library-embed .pp-header{grid-template-columns:1fr}#pp-pc-library-embed .pp-downloads,#pp-pc-library-embed .pp-session-head,#pp-pc-library-embed .pp-drawer-head{flex-direction:column;align-items:start}#pp-pc-library-embed .pp-item-main{grid-template-columns:1fr}#pp-pc-library-embed .pp-actions{min-width:0}}
  ...
  function byId(id) { return state.dataset.videos.find((v) => v.id === id); }
  function currentWorkshop() { return state.dataset.workshops.find((w) => w.id === state.activeWorkshopId); }
  ...
  function contentFor(key) {
    return contentConfig[key] || {
      ...
      summary: ["Introduces a key concept from the session.", "Provides context before reading the full transcript."],
    };
  }
  ...
  function renderDrawer(entry, video, itemConfig) {
    ...
    const left = el("div");
    left.appendChild(el("p", "pp-kicker", currentWorkshop().name));
  ...
  function render() {
    ...
    const d1 = el("a", "pp-download", "Download 1 Day PC");
    d1.href = downloads.one;
    const d2 = el("a", "pp-download secondary", "Download 2 Day PC");
    d2.href = downloads.two;
    dactions.appendChild(d1);
    dactions.appendChild(d2);
    downloadsWrap.appendChild(dcopy);
    downloadsWrap.appendChild(dactions);
    root.appendChild(downloadsWrap);

    const toolbar = el("div", "pp-toolbar");
    const tabs = el("div", "pp-tabs");
    state.dataset.workshops.forEach((w) => {
      const btn = el("button", "pp-tab" + (w.id === state.activeWorkshopId ? " active" : ""), w.name);
      btn.type = "button";
      btn.addEventListener("click", () => {
        state.activeWorkshopId = w.id;
    ...
    const sessionsWrap = el("div", "pp-sessions");
    (sessionConfig[currentWorkshop().id] || []).forEach((session) => {
      ...
      const items = el("div", "pp-items");
      let count = 0;
      session.items.forEach((itemConfig) => {
        const match = grouped.get(itemConfig.key);
        if (!match) return;
        count += 1;
        items.appendChild(renderItem(match.entry, match.video, itemConfig));
      });
      head.appendChild(el("p", "pp-session-count", `${count} videos`));
      sec.appendChild(head);
      sec.appendChild(items);
      sessionsWrap.appendChild(sec);
    });
    root.appendChild(sessionsWrap);
  }
  ...
  fetch(dataUrl)
    ...
    .catch((error) => {
      root.innerHTML = `<p style="color:#dc2d27;">Could not load the video library: ${error.message}</p>`;
    });
})();
